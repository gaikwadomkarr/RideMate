...
import 'package:bikingapp/OpenComments.dart';
...
class _PostsScreenState extends State<PostsScreen>
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  Container(
                                      ...
                                      margin: EdgeInsets.only(left: 10),
                                      child: new Text(
                                        commentsMap[postIds[outerindex]]
                                            .length
                                            .toString(),
                                        textAlign: TextAlign.center,
                                        style: TextStyle(
  ...
  Future<Null> hitPostComment(int index, StateSetter updateState) async {
    ...
    String url =
        '$commonapi/api/v1/postComments/addPostComment/' + selectedPostId;
    print(url);
    ...
    if (request.statusCode == 201) {
      ...
      print(request.data.toString());
      updateState(() {
        commentsMap.update("$selectedPostId", (value) {
          print("this is old comment value => $value");
          for (var comment in value) {
            print(comment.commentedByUserName);
          }
          print(request.data["savetoPostData"]["comments"]);
          return request.data["savetoPostData"]["comments"];
        });
      });
      print(commentsMap["$selectedPostId"]
              [commentsMap["$selectedPostId"].length - 1]
          .toString());
      // setState(() {
      //   commentsMap.update("$selectedPostId", (value) {
      //     print("this is old comment value => $value");
  ...
  void getSelectedPostsComments(String postId) {
    OpenComments(
      postId: postId,
    );
  }
